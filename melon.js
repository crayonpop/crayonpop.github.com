...
$.ajax({
    url: "http://gdata.youtube.com/feeds/api/videos/N5wzkQvzp4c?v=2&alt=json",
    dataType: "json",
    success: function (data) {
        ...
        $("#mvcut").html(urls);
        new numberCounter("count", viewcount, 0);
...
function melon() {
    PlanetX.api("get", "http://apis.skplanetx.com/melon/charts/realtime?",
        "JSON", {
            "version": 1,
            "page": 1,
            "count": 100
...
function melon_realTime_callback(data) {
    ...
    $targetNow.html("<h1>" + DateFormating(data.melon.rankDay) + "<p>-</p>" +
        data.melon.rankHour + ":00" + "</h1><br>");
    ...
    for (var i = 0; i < data.melon.songs.song.length; i++) {
        ...
        if (allRank || (artistName == catch_artist)) {
            titleEl +=
                "<tr align=center><td width=100%><b>" + songName +
                "</b></td></tr>" + "<tr align=center><td width=100%><img src='" +
                getNumberIcon(i + 1) + "' alt='" + (i + 1) + "위' />위 (" +
...
function melon_todaySong_callback(data) {
    ...
    for (var i = 0; i < data.melon.songs.song.length; i++) {
        ...
        if (allRank || (artistName == catch_artist)) {
            titleEl += "<tr align='center'>" + "<td width=10%><img src='" +
                getNumberIcon(i + 1) + "' alt='" + (i + 1) + "위' /></td>" +
                "<td width=10%>" +
                "<img style='width: 100px; height: auto;' src='" + imgLink +
                "' /></td>" + "<td width=10%>" + rankSymbol(curRank, pastRank) +
                Math.abs(pastRank - curRank) + "</td>" + "<td width=70%>" +
                songName + "</td></tr>";
        }
    }
...
function numberCounter(target_frame, target_number, nowCnt) {
    this.count = 0;
    this.diff = 0;
...
function DateFormating(day) {

    var dayString = day.toString();

    var yyyy = dayString.substring(0, 4);
    var mm = dayString.substring(4, 6);
    var dd = dayString.substring(6, 9);
